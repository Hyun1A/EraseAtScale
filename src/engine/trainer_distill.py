...
def train_erase_one_stage(
    ...
    for i in pbar:
        ...
        with autocast(dtype=amp_dtype, enabled=amp_enabled):
            ...
            cat_inputs = torch.cat([uncond, targets, mapping, anchors], dim=0).float().to(torch.bfloat16)
            ...
            aux_total = torch.tensor(0., device=device_cuda)
            for name, mod in network.named_modules():
                if getattr(mod, "use_prompt_tuning", False):
                    if hasattr(mod, "_last_aux_total"):
                        aux_total = aux_total + mod._last_aux_total

            moe_aux_coef = getattr(args, "moe_aux_coef", 1.0)
            loss["moe/aux"] = moe_aux_coef * aux_total
            ...
            loss_total = loss[f"loss_erasing"] + loss["moe/aux"]
        ...
        if i%25 == 0 and config.logging.use_wandb:
            ...
            loss = {k: v.detach().cpu().item() if type(v)==torch.Tensor else v for k, v in loss.items()}
            ...
            moe_logs = {}
            cnt = 0
            for _, mod in network.named_modules():
                if hasattr(mod, "last_aux_losses"):
                    for k, v in mod.last_aux_losses.items():
                        moe_logs[k] = moe_logs.get(k, 0.0) + float(v.item())
                    cnt += 1
            if cnt > 0:
                for k in moe_logs: moe_logs[k] /= cnt
                log_dict.update(moe_logs)            
            ################# additional log #################
