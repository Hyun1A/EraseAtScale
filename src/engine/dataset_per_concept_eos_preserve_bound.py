...
def build_triplet_loader_from_tmms(
    *,
    ...
    conf_bound = 0.8
    n_rand, conf_low, conf_high = 1024*1024, 1e-8, conf_bound

    X_target_reduced, _ = sample_in_conf_interval(
        tmm_target,
    ...
    b, d = X_target_reduced.size()
    b = b // total_token_num


    if reduce:
        X_target_recon = pca_reconstruct(X_target_reduced, tmm_target.basis, tmm_target.mean)
    ...
    X_mapping, _ = optimal_transport(tmm_target, tmm_merged, X_target_reduced, return_plan=True)
    if reduce:
    ...
    n_rand, conf_low, conf_high = 1024*1024, conf_bound, 1-1e-12

    X_anchor_reduced, _ = sample_in_conf_interval(
        tmm_target,
        n=n_rand*total_token_num,
        conf_low=conf_low,
        conf_high=conf_high,
    ...
    b, d = X_anchor_reduced.size()
    b = b // total_token_num


    if reduce:
        X_anchor_recon = pca_reconstruct(X_anchor_reduced, tmm_target.basis, tmm_target.mean)
    ...
    os.makedirs(f"./dataset/train_pairs_eos_preserve_bound_conf{conf_bound}/{replace_word}/", exist_ok=True)
    train_data_path = f"./dataset/train_pairs_eos_preserve_bound_conf{conf_bound}/{replace_word}/{target}.pt"
