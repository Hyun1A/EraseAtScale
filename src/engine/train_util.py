...
from typing import Optional, Union, Dict, List, Optional, Tuple
import gc
...
def text2img_sample_cov(pipe: DiffusionPipeline,
             prompts: Union[str, list[str]], 
             negative_prompt: Union[str, list[str]] = "", 
             width: int = 512, 
             height: int = 512,
    ...
    if generate_num == 1:
        ...
        results, latents_org, latents_noisy = pipe(
            prompts,
            negative_prompt=negative_prompt,
            width=width,
            height=height,
        ...
        texts = [f"sample/{prompt.replace(' ', '_')}{'(' + tag + ')' if tag else ''}" for prompt in prompts]
        samples = list(zip(texts, images))
        latents_dict.update({text: {"latents_org": latents_org, "latents_noisy": latents_noisy} for text in texts})
...
def select_amp_dtype(arg_amp_dtype: str | None) -> Optional[torch.dtype]:
    """Return AMP dtype from cli arg: auto->bf16 if supported else fp16."""
    if not arg_amp_dtype:
        return torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
    v = arg_amp_dtype.lower()
    if v == "off":
        return None
    if v == "bf16":
...
def enable_eval_fwd_only(unet, text_encoder, dtype: torch.dtype) -> None:
    text_encoder.to(DEVICE_CUDA, dtype=dtype).eval().requires_grad_(False)
    unet.to(DEVICE_CUDA, dtype=dtype).eval().requires_grad_(False)
    try:
...
def load_sd_components(config) -> Tuple:
    return model_util.load_models(
...
# -----------------------------------------------------------------------------
# Measurements / module mapping
# -----------------------------------------------------------------------------

def compute_lipschitz_svals(unet) -> Dict[str, List[torch.Tensor]]:
    lips_o, lips_q, lips_ov = [], [], []
    for name, module in unet.named_modules():
        if ("attn2" in name) and (module.__class__.__name__ == "Attention"):
            with torch.cuda.amp.autocast(enabled=False):
                W_o = module.to_out[0].weight.detach()
                W_q = module.to_q.weight.detach()
                W_v = module.to_v.weight.detach()
                _U, S_o, _V = torch.svd(W_o)
                _U, S_q, _V = torch.svd(W_q)
                _U, S_ov, _V = torch.svd(W_o @ W_v)
            lips_o.append(S_o[0])
            lips_q.append(S_q[0])
            lips_ov.append(S_ov[0])    
    
    return {"lipschitz_o": lips_o, "lipschitz_q": lips_q, "lipschitz_ov": lips_ov}


def map_network_to_unet_modules(network, unet) -> Dict[str, torch.nn.Module]:
    net_modules = {n: m for n, m in network.named_modules() if "EASLayer" in m.__class__.__name__}
    mapped = {}
    for name, module in unet.named_modules():
        lname = "lora_unet_" + "_".join(name.split("."))
        if lname in net_modules:
            mapped[lname] = module
    return mapped
