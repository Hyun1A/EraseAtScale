...
def train_erase_one_stage(
    ...
    for i in pbar:
        ...
        with autocast(dtype=amp_dtype, enabled=amp_enabled):
            ...
            for name in network_modules.keys():
                ...
                with torch.no_grad():
                    ...
                    crsattn_comp_org = torch.cat([crsattn_org[:n_pairs].repeat(32,1,1), crsattn_org[n_pairs:][2*n_pairs:]], dim=0)

                with network.editing():
                    crsattn = unet_modules[name](cat_inputs)
                    crsattn_target = crsattn[n_pairs:][:n_pairs]
                    crsattn_comp = torch.cat([crsattn[:n_pairs].repeat(32,1,1), crsattn[n_pairs:][2*n_pairs:]], dim=0)
        ...
        if i%5 == 0 and config.logging.use_wandb:
        ...
        if len(pbar) > 10000 and i%(len(pbar)//5) == 0:
            print("Saving...")
