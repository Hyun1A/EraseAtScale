...
if __name__ == '__main__':
    ...
    word =  " Joseph Gordon-Levitt"
    total_token_num = sum(tokenizer(word).attention_mask)- 2
    ...
    if args.reduce:
        embeds, W, mean, var = pca_reduce(embeds.to(torch.float32), k=32)
    ...
    with autocast(enabled=False):
        embeds_fp32 = embeds.detach().to(torch.float32)
        tmm = TMMTorch(
            n_components=4,
    ...
    # Visualize Earth-Mover distance
    # import matplotlib.pyplot as plt
    # from copy import deepcopy
    # total_tmms = deepcopy(tgt_tmms)
    # total_tmms.append(tmm)
    # emds = np.zeros((len(total_tmms), len(total_tmms)))
    # for i in range(len(total_tmms)):
    #     for j in range(len(total_tmms)):
    #         if i < j:
    #             emd = tmm_emd(total_tmms[i], total_tmms[j])
    #             emds[i, j] = emd
    #             emds[j, i] = emd
    ...
    prompt_gen_temp = " Joseph Gordon-Levitt"
    ...
    for conf_level in range(20):
        ...
        X_org = X_org.reshape(b, total_token_num, 768)

        X_ring, _ = optimal_transport(tmm, tmm_tgt, X_ring, return_plan=True)
        if args.reduce:
            X_ring = pca_reconstruct(X_ring, tmm_tgt.basis, tmm_tgt.mean)
        X_ring = X_ring.reshape(b, total_token_num, 768)

        span = list(range(1, total_token_num+1))

        save_path = "./images/jgl_to_ce_stack"

        prompt_gen_embeds = prompt_gen_embeds[:,:total_token_num+2]
        ...
        for rand_idx in range(n_rand):
            ...
            rand_embeds[:,span] = X_ring[rand_idx:rand_idx+1].to(torch.float16)
            images = embedding2img(rand_embeds, pipe, seed=42)[0]
            image_ot = Image.fromarray(images)

            rand_embeds = prompt_gen_embeds.clone()
            rand_embeds[:,span] = X_org[rand_idx:rand_idx+1].to(torch.float16)
            images = embedding2img(rand_embeds, pipe, seed=42)[0]
