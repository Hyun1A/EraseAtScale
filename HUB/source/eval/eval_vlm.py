...
def eval_vlm(task, method, target, seed, language=None, style=False):
    ...
    with torch.no_grad():
        for i in range(num_files):
            ...
            # # 수정: 줄을 개행으로 이어 붙여 단어 경계 보존
            # last_tail = "\n".join(response.strip().splitlines()[-3:])  # splitlines가 CRLF도 안전
            # if re.search(r"\byes\b", last_tail, re.IGNORECASE):
            #     final_answer = "yes"
            #     cnt_yes += 1
            # elif re.search(r"\bidk\b", last_tail, re.IGNORECASE):
            #     final_answer = "idk"
            #     cnt_idk += 1
            # elif re.search(r"\bwrong\b", last_tail, re.IGNORECASE):
            #     final_answer = "wrong"
            #     # 필요하면 cnt_wrong += 1 추가
            # else:
            #     final_answer = "no"
